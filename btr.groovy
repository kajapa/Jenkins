...
pipeline {
    agent {
        label {
            label 'team_x'
            customWorkspace 'C:/Jenkins/Workspace/BTR/'
        }
    }
    environment {
        def BTR_perforce = 'ssl:34.118.20.91:1666'
        def app_file = 'C:/Jenkins/steam/sdk/BTR/tools/ContentBuilder/scripts/app_2932610.vdf'
        def depot_file = 'C:/Jenkins/steam/sdk/BTR/tools/ContentBuilder/scripts/depot_2932611.vdf'
        def depot_file_with_pdb = 'C:/Jenkins/steam/sdk/BTR/tools/ContentBuilder/scripts/depot_with_pdb.vdf'
        def depot_file_without_pdb = 'C:/Jenkins/steam/sdk/BTR/tools/ContentBuilder/scripts/depot_without_pdb.vdf'
        def commit_file = 'C:/Jenkins/Workspace/BTR_commit_id.txt'
            ...
            when
      {
        expression
        {
          return skip_build == "false"
            ...
            steps {
                script {
                    def folder_name = "C:/Builds/BTR/${P4_CHANGELIST}_BTR_${conf_short}"
                    echo"Nazwa folderu $folder_name"
                    if (env.checkout_clean == 'Yes') {
                        bat """
"D:/Program Files/Epic Games/UE_5.3/Engine/Build/BatchFiles/RunUAT.bat" BuildCookRun -rocket -clean -pak -compile -compileeditor -installed -project="C:/Jenkins/Workspace/BTR/BeTheRuler/BeTheRuler.uproject" -nop4 -utf8output -buildmachine -platform=Win64 -targetplatform=Win64 -clientconfig=%env.build_conf% -build -cook -stage -verbose -NoSign -NoCodeSign -stagingdirectory="${folder_name}" -log="C:/Jenkins/Workspace/BTR/Logs/BTR_Log.txt"
"""
         }else if (env.checkout_clean == 'No') {
                        bat """
"D:/Program Files/Epic Games/UE_5.3/Engine/Build/BatchFiles/RunUAT.bat" BuildCookRun -rocket -pak -compile -compileeditor -installed -project="C:/Jenkins/Workspace/BTR/BeTheRuler/BeTheRuler.uproject" -nop4 -utf8output -buildmachine -platform=Win64 -targetplatform=Win64 -clientconfig=%env.build_conf% -build -cook -stage -verbose -NoSign -NoCodeSign -stagingdirectory="${folder_name}" -log="C:/Jenkins/Workspace/BTR/Logs/BTR_Log.txt"
"""
                    }
    }}
            }
            ...
            when
      {
        expression
        {
          return skip_build == "false"
            ...
            steps {
                script {
                    def build_path = "C:/Builds/BTR/${P4_CHANGELIST}_BTR_${conf_short}/Windows/"
                    def compressed_path = "C:/Builds_Compressed/BTR/${P4_CHANGELIST}_BTR_${conf_short}.zip"
...
"""
            ...
            steps {
                script {
                    ...
                    if (env.buid_type == 'Latest') {
                        ...
                        def newContent2 = readFile(depot_file).replaceAll(/.*"contentroot".*/, "\"contentroot\" \"C:\\\\Builds\\\\BTR\\\\${COMBINED_VERSION}\\\\Windows\\\"")
                        ...
                        withCredentials([usernamePassword(credentialsId: 'buildmachine_password', usernameVariable: 'USERNAME', passwordVariable: 'PASSWORD')]) {
                            bat """C:/Jenkins/steam/sdk/BTR/tools/ContentBuilder/builder/steamcmd.exe +login ${USERNAME} "${PASSWORD}" +run_app_build "C:/Jenkins/steam/sdk/BTR/tools/ContentBuilder/scripts/app_2932610.vdf" +quit"""
                        }
    ...
    post {
        ...
        success {
            script {
                def p4Command = "p4 -u mosica -p ${BTR_perforce} -s ${P4_CHANGELIST}"
    ...
    //echo "Wynik komendy p4: ${p4Result}"
                ...
                slackSend botUser: true, channel: '#ruler-ci-cd', color: '#00FF00', message: "<@U051VA48N0P><@U0525KMA6HW><@U05UJ2T4UNA> New build available: $P4_CHANGELIST BTR Commit description:\n*${finalFilteredText}* <https://drive.google.com/drive/folders/12q_vPVj8mX5FtejXEyDLNIjiWTbFL-jL?usp=drive_link|BTR folder>", tokenCredentialId: 'BTR_bot'
            }
        }
        failure {
            script {
                def p4Command = "p4 -u mosica -p ${BTR_perforce} describe -s ${P4_CHANGELIST}"
                ...
                def p4Command2 = "p4 -u mosica -p ${BTR_perforce} describe -s ${P4_CHANGELIST}| findstr @"
                ...
                slackSend botUser: true, channel: '#ruler-ci-cd', color: '#FF0000', message: "<@UE4BVMQCT><@U0173MLGL0H><@U0525KMA6HW><@U05UJ2T4UNA> Build failed at commit: $P4_CHANGELIST Commit description:\n*${finalFilteredText}*\n submitted by *${submitter}* Link to <http://192.168.100.137:8080/job/${JOB_NAME}/${BUILD_NUMBER}/artifact/|artefacts> ${cleanedContent}", tokenCredentialId: 'BTR_bot'
            }
        }
    }
        }
