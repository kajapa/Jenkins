//parameters
def REPOSITORY_DIR_PATH = "D:\\Jenkins\\Workspace\\NED"
def GAME_CONFIG_INI_PATH = "D:/Jenkins/Workspace/NED/NeverEndingDungeon/Config/DefaultGame.ini"
def ENGINE_CONFIG_INI_PATH = "D:/Jenkins/Workspace/NED/NeverEndingDungeon/Config/DefaultEngine.ini"
...
def TriggerBuild(InClean, InArgs, InFolderName, InBuildConf)
{
  ...
  {
    bat """
"C:/Program Files/Epic Games/UE_5.4/Engine/Build/BatchFiles/RunUAT.bat" BuildCookRun -rocket -clean -pak -compile -CloudDir="D:/Jenkins/Workspace/NED/Cloud" -compileeditor -installed -project="D:/Jenkins/Workspace/NED/NeverEndingDungeon/NeverEndingDungeon.uproject" -nop4 -utf8output  -buildmachine -platform=Win64 -targetplatform=Win64 -clientconfig="${InBuildConf}" -build -cook -stage -verbose -NoSign -NoCodeSign -stagingdirectory="${InFolderName}" -ubtargs="${InArgs}" -log="D:/Jenkins/Workspace/NED/Logs/NED_Log.txt"
"""  
  }
  else if(InClean == 'No')
  {
    bat """
"C:/Program Files/Epic Games/UE_5.4/Engine/Build/BatchFiles/RunUAT.bat" BuildCookRun -rocket -pak -compile -CloudDir="D:/Jenkins/Workspace/NED/Cloud" -compileeditor -installed -project="D:/Jenkins/Workspace/NED/NeverEndingDungeon/NeverEndingDungeon.uproject" -nop4 -utf8output  -buildmachine -platform=Win64 -targetplatform=Win64 -clientconfig="${InBuildConf}" -build -cook -stage -verbose -NoSign -NoCodeSign -stagingdirectory="${InFolderName}" -ubtargs="${InArgs}" -log="D:/Jenkins/Workspace/NED/Logs/NED_Log.txt"
"""  
  }   
}
...
{
  bat"""
"D:/Build Patch Tool/Engine/Binaries/Win64/BuildPatchTool.exe" -OrganizationId="o-tjbpeg9z2wuydsmqfuf3bjqtn59g7w" -ProductId="44a316f76dd34389877a9983da0b996d" -ArtifactId="4e9dc82133b64c18bbd17d67f4a751e3" -ClientId="xyza7891aSr3VDAOdP6L6TGFhawNAVJm" -ClientSecret="4RbGuuMrG3PSIXtZf4OrSBRe27DzFCfkKMw6qT4Rv2w" -mode=UploadBinary -BuildRoot="${InBuildPath}" -CloudDir="D:/Jenkins/Workspace/NED_Cloud" -BuildVersion="${InCombinedVersion}" -AppLaunch="NeverEndingDungeon.exe" -Platform="Windows" -AppArgs="" -FileAttributeList="" -FileIgnoreList=""
"""
}
  ...
  stages
  {
    ...
    stage('Prepare: Steam')
    {
      ...
      steps
      {
        script
        {
          def gameConfigCheckoutCommand = "p4 -u qa -p ssl:34.118.124.6:1666 -c jenkins-NED-Node1 edit -c default ${GAME_CONFIG_INI_PATH}"
          def engineConfigCheckoutCommand = "p4 -u qa -p ssl:34.118.124.6:1666 -c jenkins-NED-Node1 edit -c default ${ENGINE_CONFIG_INI_PATH}"  
    ...
    stage('Build: Steam')
    {
      ...
      steps
      {
        script
        {
          ...
          def gameConfigCheckoutCommand = "p4 -u qa -p ssl:34.118.124.6:1666 -c jenkins-NED-Node1 revert ${GAME_CONFIG_STREAM_PATH}"
          def engineConfigCheckoutCommand = "p4 -u qa -p ssl:34.118.124.6:1666 -c jenkins-NED-Node1 revert ${ENGINE_CONFIG_STREAM_PATH}"  
    ...
    stage('Upload: Steam')
    {
      ...
      steps
      {
        script
        {
          ...
          def branch =  readFile(app_1).replaceAll(/.*"setlive".*/, "\"setlive\" \"${env.steam_branch}\"")
  ...
  post
  {
    always
    {
      script
      {
        ...
        dir("C:/Users/Master/AppData/Roaming/Unreal Engine/AutomationTool/Logs/C+Program+Files+Epic+Games+UE_5.4")
        {
          archiveArtifacts '*.txt'
        }

        bat 'del /q /s "C:\\Users\\Master\\AppData\\Roaming\\Unreal Engine\\AutomationTool\\Logs\\C+Program+Files+Epic+Games+UE_5.4\\*"'
        }
      }
    }
    success
